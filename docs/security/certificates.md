...
determining the capabilities to enable.

## Console Certificate

Per-console certificate to verify and define the device. Stored in
**sp_s.cfg** (offset: 0x5400) inside [XBFS](../boot/xbox-boot-file-system.md).
...
Stored in **certkeys.bin** inside [XBFS](../boot/xbox-boot-file-system.md).
...
enum CERTIFICATE_CAPABILITIES : ushort {
    ...
    VIRT_CAP_MS_DEVKIT_EQUIV = 0xF004,
    VIRT_CAP_RED_SIGNED_CPU_CODE = 0xF005
};
```
